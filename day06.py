...
def test_ways_to_beat_record():
    assert [ways_to_beat_record(race_time, record) for race_time, record in TEST_INPUT] == [4, 8, 9]


def part1(times_records):
    return math.prod(ways_to_beat_record(race_time, record) for race_time, record in times_records)
...
#
# if __name__ == "__main__":
#     answer = part2(file_lines("day06_input.txt"))
